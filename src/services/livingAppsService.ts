...
import type { Objekte, Maengel, CreateObjekte, CreateMaengel } from '@/types/app';
...
export class LivingAppsService {
  // --- OBJEKTE ---
  static async getObjekte(): Promise<Objekte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.OBJEKTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Objekte[];
    return enrichLookupFields(records, 'objekte');
  }
  static async getObjekteEntry(id: string): Promise<Objekte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.OBJEKTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Objekte;
    return enrichLookupFields([record], 'objekte')[0];
  }
  static async createObjekteEntry(fields: CreateObjekte) {
    return callApi('POST', `/apps/${APP_IDS.OBJEKTE}/records`, { fields: cleanFieldsForApi(fields as any, 'objekte') });
  }
  static async updateObjekteEntry(id: string, fields: Partial<CreateObjekte>) {
    return callApi('PATCH', `/apps/${APP_IDS.OBJEKTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'objekte') });
  }
  static async deleteObjekteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.OBJEKTE}/records/${id}`);
  }

  // --- MAENGEL ---
  static async getMaengel(): Promise<Maengel[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MAENGEL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Maengel[];
    return enrichLookupFields(records, 'maengel');
  }
  static async getMaengelEntry(id: string): Promise<Maengel | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MAENGEL}/records/${id}`);
    const record = { record_id: data.id, ...data } as Maengel;
    return enrichLookupFields([record], 'maengel')[0];
  }
  static async createMaengelEntry(fields: CreateMaengel) {
    return callApi('POST', `/apps/${APP_IDS.MAENGEL}/records`, { fields: cleanFieldsForApi(fields as any, 'maengel') });
  }
  static async updateMaengelEntry(id: string, fields: Partial<CreateMaengel>) {
    return callApi('PATCH', `/apps/${APP_IDS.MAENGEL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'maengel') });
  }
  static async deleteMaengelEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MAENGEL}/records/${id}`);
  }
