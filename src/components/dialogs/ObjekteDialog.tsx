...
import type { Objekte, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Objekte';
...
interface ObjekteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Objekte['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Objekte['fields'], 'belegungsstatus'> & {
    belegungsstatus?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  belegungsstatus: LOOKUP_OPTIONS['objekte']?.['belegungsstatus'] ?? [],
};
...
export function ObjekteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ObjekteDialogProps) {
  const [fields, setFields] = useState<Partial<Objekte['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'strasse', 'hausnummer', 'postleitzahl', 'stadt', 'standort', 'belegungsstatus'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Objekte['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'objekte');
      await onSubmit(clean as Objekte['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Objektbezeichnung\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "postleitzahl": string | null, // Postleitzahl\n  "stadt": string | null, // Stadt\n  "belegungsstatus": LookupValue | null, // Belegungsstatus (select one key: "vermietet" | "leer_stehend" | "in_renovierung") mapping: vermietet=Vermietet, leer_stehend=Leer stehend, in_renovierung=In Renovierung\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Objekte['fields']>;
      });
      if (gps) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Objekte bearbeiten' : 'Objekte hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        <Label htmlFor="bezeichnung">Objektbezeichnung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bezeichnung"
          placeholder="z. B. Musterstr. 5"
    ...
    'standort': (
      <div key="standort" className="space-y-1.5">
        <Label htmlFor="standort">Standort <span className="text-destructive" aria-hidden="true">*</span></Label>
        ...
        {showErrors && !fields.standort && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'belegungsstatus': (
      <div key="belegungsstatus" className="space-y-1.5">
        <Label htmlFor="belegungsstatus">Belegungsstatus <span className="text-destructive" aria-hidden="true">*</span></Label>
        ...
        {showErrors && !fields.belegungsstatus && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Renovierungshinweise, Zustand der Räume..."
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Objektbezeichnung", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "stadt": "Stadt", "standort": "Standort", "belegungsstatus": "Belegungsstatus", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.OBJEKTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
