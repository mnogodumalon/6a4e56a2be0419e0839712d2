...
import { ObjekteViewDialog } from '@/components/dialogs/ObjekteViewDialog';
import { MaengelDialog } from '@/components/dialogs/MaengelDialog';
import { MaengelViewDialog } from '@/components/dialogs/MaengelViewDialog';
...
const OBJEKTE_FIELDS = [
  { key: 'bezeichnung', label: 'Objektbezeichnung', type: 'string/text' },
  ...
  { key: 'standort', label: 'Standort', type: 'geo' },
  { key: 'belegungsstatus', label: 'Belegungsstatus', type: 'lookup/radio', options: [{ key: 'vermietet', label: 'Vermietet' }, { key: 'leer_stehend', label: 'Leer stehend' }, { key: 'in_renovierung', label: 'In Renovierung' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const MAENGEL_FIELDS = [
  { key: 'objekt', label: 'Objekt', type: 'applookup/select', targetEntity: 'objekte', targetAppId: 'OBJEKTE', displayField: 'bezeichnung' },
  { key: 'mangelbezeichnung', label: 'Mangelbezeichnung', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'prioritaet', label: 'Priorität', type: 'lookup/radio', options: [{ key: 'mittel', label: 'Mittel' }, { key: 'hoch', label: 'Hoch' }, { key: 'dringend', label: 'Dringend' }, { key: 'niedrig', label: 'Niedrig' }] },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'behoben', label: 'Behoben' }] },
  { key: 'meldedatum', label: 'Meldedatum', type: 'date/date' },
  { key: 'zustaendige_person', label: 'Zuständige Person', type: 'string/text' },
  { key: 'kontakt', label: 'Kontakt (Telefon oder E-Mail)', type: 'string/text' },
  { key: 'anhaenge', label: 'Fotos / Dokumente', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'objekte', label: 'Objekte', pascal: 'Objekte' },
  { key: 'maengel', label: 'Mängel', pascal: 'Maengel' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('objekte');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objekte': return (data as any).objekte as Objekte[] ?? [];
      case 'maengel': return (data as any).maengel as Maengel[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'maengel':
        lists.objekteList = (data as any).objekte ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'maengel' && fieldKey === 'objekt') {
      const match = (lists.objekteList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objekte': return OBJEKTE_FIELDS;
      case 'maengel': return MAENGEL_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objekte': return {
        create: (fields: any) => LivingAppsService.createObjekteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateObjekteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteObjekteEntry(id),
      };
      case 'maengel': return {
        create: (fields: any) => LivingAppsService.createMaengelEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMaengelEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMaengelEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'objekte' || dialogState?.entity === 'objekte') && (
        <ObjekteDialog
          open={createEntity === 'objekte' || dialogState?.entity === 'objekte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'objekte' ? handleUpdate : (fields: any) => handleCreate('objekte', fields)}
          defaultValues={dialogState?.entity === 'objekte' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Objekte']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Objekte']}
        />
      )}
      {(createEntity === 'maengel' || dialogState?.entity === 'maengel') && (
        <MaengelDialog
          open={createEntity === 'maengel' || dialogState?.entity === 'maengel'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'maengel' ? handleUpdate : (fields: any) => handleCreate('maengel', fields)}
          defaultValues={dialogState?.entity === 'maengel' ? dialogState.record?.fields : undefined}
          objekteList={(data as any).objekte ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Maengel']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Maengel']}
        />
      )}
      {viewState?.entity === 'objekte' && (
        <ObjekteViewDialog
          open={viewState?.entity === 'objekte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'objekte', record: r }); }}
        />
      )}
      {viewState?.entity === 'maengel' && (
        <MaengelViewDialog
          open={viewState?.entity === 'maengel'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'maengel', record: r }); }}
          objekteList={(data as any).objekte ?? []}
        />
      )}
